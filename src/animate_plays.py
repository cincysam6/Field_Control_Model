...
class AnimatePlayWithDensity:
    ...
    def _update(self, _frame_index: int):
        """Advance to the next frame: update positions, contours, labels, and arrows."""
        # Advance stream; reset if exhausted so animation can loop
        try:
        ...
        # Clear old density contours
        self._clear_contours_safe(self._contours)

        # Plot densities for this frame (if provided)
        if self.show_contours:
            ...
            for _, prow in frame_density_df.iterrows():
                ...
                if Z is None:
                    continue
                cmap = self.on_cmap if int(prow.get("is_off", 0)) == 1 else self.off_cmap
                Z_masked = np.where(Z > 0.01, Z, np.nan)  # soft mask for cleanliness
                cont = self._ax_field.contourf(
                    self._X, self._Y, Z_masked, cmap=cmap, levels=self.contour_levels, alpha=self.contour_alpha
                )
                self._contours.append(cont)

        # Per-player labels + arrows (only where jerseyNumber is present)
        ...
        for idx in range(min(len(labeled), self._MAX_FIELD_PLAYERS)):
            ...
            # --- arrow heading (prefer precomputed theta if available) ---
            theta_rad = None
            ...
            if theta_rad is None:
                # fall back to mapping row.dir with bias
            ...
            self._a_dir_list[idx] = self._ax_field.add_patch(
                Arrow(
                    ...
                    L * np.sin(theta_rad),
                    color="k",
                    width=self.arrow_tail_width,
                    alpha=1.0,
                    zorder=7,
                )
            )

            # Replace orientation arrow (grey), if orientation column exists
            ...
            if ("o" in row) and (row["o"] == row["o"]):  # pd.notna without importing pandas here
                ...
                self._a_or_list[idx] = self._ax_field.add_patch(
                    Arrow(
                        x0,
                        y0,
                        (L * 0.6) * np.cos(orient_theta),
                        (L * 0.6) * np.sin(orient_theta),
                        color="grey",
                        width=self.arrow_tail_width * 1.2,
                        alpha=0.95,
                        zorder=6,
                    )
                )
            else:
                # keep list aligned; insert a “no-op” arrow off-canvas
        ...
        # Return a tuple of artists (Matplotlib uses this for blitting if enabled)
        return (
            self._scat_ball,
            self._scat_offense,
            self._scat_defense,
